...
import cn.dev4mob.app.utils.AnimationUtils;
...
public class SpotlightActivity extends Activity {
	...
	private void createAnimation(final SpotlightView spotlight) {
		...
		spotlight.animate().alpha(1.0f).setListener(AnimationUtils.withEndAction( new Runnable() {
      @Override
      public void run() {
        ...
        ObjectAnimator scaleUp =
            ObjectAnimator.ofFloat(spotlight, "maskScale", startScale, startScale * 3.0f);
        scaleUp.setDuration(2000);

        ObjectAnimator moveCenter =
            ObjectAnimator.ofFloat(spotlight, "maskX", spotlight.getWidth() / 2.0f);
        moveCenter.setDuration(1000);

        ObjectAnimator moveUp =
            ObjectAnimator.ofFloat(spotlight, "maskY", spotlight.getHeight() / 2.0f);
